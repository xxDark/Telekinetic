...
final class ASMEventExecutor {
    ...
    EventExecutor<?> generateExecutor(Object listener, Method method, ClassLoader loader) {
        ...
        val executorName = "ru/xdark/telekinetic/event/EventExecutor$$" + owner.getName().replace('.', '$') + "__" + methodName;
        writer.visit(JavaUtil.vmVersion() + 44, ACC_FINAL, executorName, null, BRIDGE_INTERNAL, new String[]{"ru/xdark/telekinetic/event/EventExecutor"});
        ...
        mv.visitMaxs(2, 2);
        mv.visitEnd();
        mv = writer.visitMethod(ACC_PUBLIC, "dispatch", "(Lru/xdark/telekinetic/event/Event;)V", null, null);
        mv.visitCode();
        mv.visitVarInsn(ALOAD, 0);
        ...
        val klass = JavaUtil.unsafe().defineClass(executorName, bytes, 0, bytes.length, loader, null);
